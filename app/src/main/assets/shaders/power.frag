...
//  vec4 albedoColor2 =texture(u_AlbedoTexture, vec2(v_TexCoord.x- u_time, 1.0 - v_TexCoord.y));
//
//  albedoColor.rgb = mix(albedoColor, albedoColor2, 0.5);
  vec4 albedoColor = texture(u_AlbedoTexture, vec2(v_TexCoord.x, 1.0 - v_TexCoord.y));

  float t = 0.5 + ((u_time<0.5) ? u_time : (1.0 - u_time));
  albedoColor.rgb *= u_AlbedoColor* t;
  ...
  o_FragColor.a = max(0.0, 1.0 - u_time);
